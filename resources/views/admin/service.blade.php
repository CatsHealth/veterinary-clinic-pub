    ...
    <div class="modal-content">
        ...
        <form class="service-form" id="editServiceForm" method="POST" action="{{ route('service.update', '') }}">
            ...
            <div class="form-group service-doctor">
                <label for="edit_doctor">Врачи:</label>
                <select id="edit_doctor" name="doctor_ids[]" class="service-doctor-select" multiple required>
                    @foreach($doctors as $doctor)
                        <option value="{{ $doctor->id }}" 
                            @if(in_array($doctor->id, old('doctor_ids', $service->doctors->pluck('id')->toArray())))
                                selected
                            @endif
                        >{{ $doctor->name }}</option>
                    @endforeach
